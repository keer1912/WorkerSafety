...
// Based on existing bidirectional ESP-NOW implementation with added mesh capabilities
...
const char* NODE_NAME = "SecNode1"; // Human-readable name
...
void initMesh() {
  ...
  for (int i = 0; i < MAX_MESH_PEERS; i++) {
    ...
    knownPeers[i].isEncrypted = false;
    knownPeers[i].keyIndex = 0;
  }
...
void updatePeerInfo(const uint8_t *mac, uint8_t nodeId, uint16_t keyIndex) {
  int emptySlot = -1;
