...
def on_message(client, userdata, msg):
    ...
    if len(topic_parts) >= 5:  # Make sure we have enough parts
        ...
        if sensor_type == "falldetect":
            ...
            print(f"Worker {worker_id} on floor {floor_id} at site {site_id} has heart rate: {msg.payload.decode()} bpm")
        elif sensor_type == "battery":
            print(f"Worker {worker_id} on floor {floor_id} at site {site_id} has battery level: {msg.payload.decode()}%")
