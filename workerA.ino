...
void sendFallEmergency() {
  ...
  sprintf(outgoingData.message, "EMERGENCY: WORKER A FALLEN!");
  ...
  if (mqttConnected) {
    mqttClient.publish(topic_fall, "Worker A Fall Detected!");
  }
}
...
void OnDataRecv(const uint8_t *mac_addr, const uint8_t *data, int data_len) {
  ...
  // Check if this is an emergency notification from Worker B
  ...
  switch (userState) {
    ...
    case FALLEN:
      ...
      if (M5.BtnA.wasPressed()) {
        ...
        if (mqttConnected) {
          mqttClient.publish(topic_recovery, "Worker A has recovered from fall");
        }
...
void loop() {
  ...
  if (M5.BtnB.wasPressed() && userState != FALLEN) {
    ...
    sprintf(outgoingData.message, "MANUAL ALERT: WORKER A NEEDS HELP!");
    outgoingData.value = 0.0;
    outgoingData.keyIndex = keyMgr.keyIndex;
    outgoingData.heartRate = heartRate;
    outgoingData.batteryLevel = batteryLevel;
    ...
    if (mqttConnected) {
      mqttClient.publish(topic_fall, "Worker A Manual Emergency Alert!");
    }
